...
| **2026.9.3** | 2026-09-02 | SSRF 防护出站网络安全网关（`fetchWithSsrFGuard`）+ PIN 消息操作工具支持 + 引入 vitest 单元测试套件 |
| **2026.9.2** | 2026-09-01 | 首发 ClawHub 正式版，`ask_user` 交互卡片按钮提交回退机制 |
| **2026.9.1** | 2026-09-01 | OpenClaw 2.0 SDK 初始适配（SDK 导出路径修正、SQLite 数据结构迁移、旧版插件兼容补丁）|
