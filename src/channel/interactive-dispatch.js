...
function extractBasics(data) {
    try {
        const ev = data;
        const action = ev.action?.value?.action ?? ev.action?.name ?? formSubmitActionName(ev);
...
function formSubmitActionName(ev) {
    const tag = ev.action?.tag;
    if (tag !== 'form_submit')
        return undefined;
    return ev.action?.form_name;
}
